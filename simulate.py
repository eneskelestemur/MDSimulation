...
protein_files = [f'{output_dir}/ras.pdb', f'{output_dir}/raf.pdb']
ligand_files = []
rna_files = []
...
complex_sim.run_simulation(
    ...
    sim_reporters=[app.StateDataReporter(f'{output_dir}/sim_data.log', 1000, step=True, potentialEnergy=True, totalEnergy=True, temperature=True, density=True), 
                   app.DCDReporter(f'{output_dir}/sim_trajectory.dcd', 20000),
    ...
    equil_reporters=[app.StateDataReporter(f'{output_dir}/equil_data.log', 100, step=True, potentialEnergy=True, totalEnergy=True, temperature=True, density=True), 
                     app.DCDReporter(f'{output_dir}/equil_trajectory.dcd', 2000),
    ...
    solvent_kwargs={'model': 'tip3p', 'padding': 1.0*unit.nanometer, 'positiveIon': 'Na+',
                    'negativeIon': 'Cl-', 'ionicStrength': 0.05*unit.molar, 'neutralize': True,},
...
Simulation.plot_RMSD(f'{output_dir}/equil_trajectory.dcd', f'{output_dir}/minimized_complex.pdb',
                        labels=['Backbone', 'Protein', 'Ligand', 'RNA', r'$C_{\alpha}$'],
                        rmsd_kwargs={'select': 'backbone', 'groupselections': ['segid A', 'segid B', 'segid C', 'name CA']}, 
                        save=True)
